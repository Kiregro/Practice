...
Помимо прочего, в массиве атрибуту "name" присваивается значение "cvc". Данный атрибут избыточен для трекера, поскольку его заявленное назначение - замер времени, потраченного пользователем до момента отправки перевода.

3. Следом за массивом идет, предположительно, функция-декодер _0xd81a, т.к. она:
   1. Принимает число;
   2. Осуществляет арифметическую операцию;
   3. Обращается к массиву;
   4. Возвращает строку;
   5. Используется далее по коду для присваивания значений переменным внутри функции, которые потом используются с различными индексами.
